...
def split_strings_from_page(
    ...
    # if recursion hasn't found a split after X iterations, just truncate
        ...
        return [truncated_string(page, model=model, max_tokens=max_tokens)]
    # otherwise, split in half and recurse
    else:
        # titles, text = subsection
        for delimiter in ["\n\n", "\n", ". "]:
            left, right = halved_by_delimiter(page, delimiter=delimiter)
            if left == "" or right == "":
                ...
                for half in [left, right]:
                    ...
                    half_strings = split_strings_from_page(
                        half,
...
page_strings = []
for pp in pages:
    page_strings.extend(split_strings_from_page(pp.page_content, max_tokens=MAX_TOKENS))


print(f"{len(pages)} pages split into {len(page_strings)} strings.")

# print example data
print(page_strings[1])
...
embeddings = []
for batch_start in range(0, len(page_strings), BATCH_SIZE):
    batch_end = batch_start + BATCH_SIZE
    batch = page_strings[batch_start:batch_end]
...
df = pd.DataFrame({"text": page_strings, "embedding": embeddings})
